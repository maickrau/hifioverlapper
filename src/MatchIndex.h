...
public:
	...
	void addSeen(uint64_t key);
	void clearSeen();
	void addNumber(uint64_t key, __uint128_t value);
	void initializeBuckets();
	size_t seenSize() const;
	size_t bucketsSize() const;
private:
	phmap::parallel_flat_hash_set<uint64_t> seenOnce;
	phmap::parallel_flat_hash_map<uint64_t, uint32_t> nonsingletonVectorIndex;
...
public:
	...
	void clearSeenAndInitializeBuckets();
	void clearConstructionVariablesAndCompact();
	void countSeenHashes(std::mutex& indexMutex, const std::string& readSequence);
